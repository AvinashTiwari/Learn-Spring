...
public class CreditCardApplicationContoller {
    ...
    public ApplyForCreditCardResponse applyForCreditCard(final ApplyForCreditCardRequest appliedForCreditCardRequest){
        final int citizenNumber = appliedForCreditCardRequest.getCitizenNumber();
       final CreditCheckResponse creditCheckResponse =  restTemplate.postForObject("http://localhost:8080/credit-score", new CreditRequestCheck(citizenNumber), CreditCheckResponse.class);
